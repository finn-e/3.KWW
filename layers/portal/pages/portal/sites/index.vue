...
<template>
	<div>
		<PortalPageHeader title="Sites" :breadcrumbs="[
			{
				title: 'Portal',
				href: '/portal',
			},
			...
			<template #actions>
				<UButton color="primary" variant="outline" size="xl" :loading="stripeLoading"
			...
			<template #header>
				...
				<div class="flex items-center justify-between gap-3">
					...
					<div class="flex gap-3">
						...
						<UButton color="white" size="xs" :disabled="!search && !status"
							icon="material-symbols:filter-alt-off-outline-rounded" @click="clearFilters">
							Reset
						</UButton>
					</div>
				</div>
			...
			<UTable v-auto-animate :columns="columns" :rows="invoices" column-attribute="label" :loading="pending">
				...
				<template #empty-state>
					<div class="w-1/4 mx-auto text-center">
						...
						<UButton v-if="search || status" color="white" size="xs"
							icon="material-symbols:filter-alt-off-outline-rounded" class="mt-4" @click="clearFilters">
							Reset Filters
						</UButton>
					</div>
				</template>
				...
				<template #status-data="{ row }">
					<UBadge :label="row.status" :color="row.status === 'unpaid' ? 'rose' : 'primary'" size="xs"
						class="capitalize" />
				</template>
				...
				<template #actions-data="{ row }">
					<UButton :to="`/portal/billing/invoices/${row.id}`" color="primary" variant="outline"
